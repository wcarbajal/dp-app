...
import { ArrowLeft, Plus, Edit, Trash, CloudUpload, Refresh } from "tabler-icons-react";
...
export const ResultadosIndicadorPage = () => {
  ...
  const handleEdit = ( resultado ) => {
    ...
    const isAccordionClosed = accordionValue !== "formulario-resultado";

    if ( isAccordionClosed ) {
      // Si está cerrado, primero abrirlo
      setAccordionValue( "formulario-resultado" );

      // Esperar más tiempo para que se complete la animación de apertura
      setTimeout( () => {
      ...
      }, 300 ); // Tiempo aumentado para permitir la animación de apertura
    } else {
      // Si ya está abierto, hacer scroll inmediatamente
      setTimeout( () => {
        document.getElementById( 'formulario-resultado' )?.scrollIntoView( {
          behavior: 'smooth',
          block: 'start'
