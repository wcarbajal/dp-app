...
export const FormProcesoConfig = ( { procesos, idMapa = 1, editId, setOpenDialog, setEditId } ) => {
  ...
  const onSubmit = async ( data ) => {
    try {
      ...
      if ( editId ) {
        respuesta = await fetchConToken( `procesos/${ editId }`, data, "PUT" );
      } else {
        respuesta = await fetchConToken( `procesos/1/registrar`, data, "POST" );
      }
      if ( respuesta.ok ) {
